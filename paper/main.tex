...
\usepackage{url}
\usepackage{booktabs}
\title{Open Robot Data Coverage Gaps: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about open robot datasets. The original research bet was: Measure mechanism coverage gaps in open robot data beyond task and embodiment counts. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item PaPaGei: Open Foundation Models for Optical Physiological Signals (2024)
\item A Hierarchical Decoupling Task Planning Method for Multi-UAV Collaborative Multi-Region Coverage with Task Priority Awareness (2025)
\item Open X-Embodiment: Robotic Learning Datasets and RT-X Models : Open X-Embodiment Collaboration 0 (2024)
\item Open X-Embodiment: Robotic Learning Datasets and RT-X Models (2023)
\item Denoising task-correlated head motion from motor-task fMRI data with multi-echo ICA (2024)
\end{itemize}
